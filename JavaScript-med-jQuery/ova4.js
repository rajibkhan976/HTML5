...
//console.log(point);
//15
var amountPurchased = window.prompt("Put the amount purchased by the customer");
var amountPaid = window.prompt("Put the amount paid by the customer");
var returncurrency = [1000, 500, 200, 100, 50, 20, 10, 5, 2, 1];
var determinedCurrency = [];
var modulas, note = 0, sum = 0;
if(amountPaid > amountPurchased) {
  var amountReturn = amountPaid - amountPurchased;
  for (var l = 0; l < returncurrency.length; l++) {
    if(amountReturn < returncurrency[l]) {
      continue;
    }
    modulas = amountReturn % returncurrency[l];
    if(modulas != 0) {
    determinedCurrency.push(returncurrency[l]);
    }
  }
}
    for (var k = 0; k < determinedCurrency.length; k++) {
      modulas = amountReturn % determinedCurrency[k];
      if(modulas != 0) {
        amountReturn = modulas;
      }
      sum = sum + determinedCurrency[k];
    }
